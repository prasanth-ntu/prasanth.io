...
const STATIC_SLIDES_DIR = '../../../../../../quartz/static/slides';
  ...
  <body>
    ...
    <div class="slideshow-container">
      <div class="loading" id="loading">
        <i class="fas fa-spinner fa-spin"></i> Loading slides...
      </div>
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        currentSlideIndex = getSlideFromURL();
        showSlide(currentSlideIndex);
...
function getSlidesList() {
  try {
    ...
    console.error('Error reading slides directory:', error.message);
    return [];
  }
}

function generateIndicators(totalSlides) {
  let indicators = '';
  for (let i = 0; i < totalSlides; i++) {
    indicators += `<div class="indicator" onclick="goToSlide(${i})" title="Go to slide ${i + 1}"></div>\n      `;
  }
  return indicators.trim();
}

function generateSlideIframes(slides) {
  let iframes = '';
  slides.forEach((slide, index) => {
    const slideName = path.basename(slide, '.html');
    const slideUrl = `/static/slides/${slideName}`; // Use static directory with clean URLs (no .html)
...
function buildSlideshow() {
  console.log('🔍 Scanning slides directory...');
  
  const slides = getSlidesList();
  const totalSlides = slides.length;
  
  if (totalSlides === 0) {
    console.error('❌ No HTML files found in slides directory');
    return;
  }
  ...
  const slideIframes = generateSlideIframes(slides);
  
  // Replace template placeholders
  let output = SLIDESHOW_TEMPLATE
    .replace(/{{TITLE}}/g, 'Slideshow Presentation')
    .replace(/{{TOTAL_SLIDES}}/g, totalSlides)
    .replace(/{{INDICATORS}}/g, indicators)
    .replace(/{{SLIDE_IFRAMES}}/g, slideIframes);
  
  // Write output file
  try {
    fs.writeFileSync(OUTPUT_FILE, output);
    console.log(`✅ Slideshow built successfully: ${OUTPUT_FILE}`);
    console.log(`🎯 Open ${OUTPUT_FILE} in your browser to view the presentation`);
